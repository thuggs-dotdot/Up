...
local _remotes = ReplicatedStorage:WaitForChild("Remotes", 9e9)

local MainPage = Window:NewPage({Title = "Main", Icon = "rbxassetid://74730846535909", TabImageColor = "#000000"})
        ...
        while getgenv().AutoSukli do
            ...
            for _, btn in pairs(playerGui:GetDescendants()) do
                if (btn:IsA("TextButton") or btn:IsA("ImageButton")) and btn.Visible then
                    ...
                    local tText = ""
                    if btn:IsA("TextButton") then tText = string.lower(btn.Text) end
...
MainPage:Section("Auto Cash & Sukli")
                ...
                task.spawn(function()
                    while getgenv().AutoCoins do
                        ...
                        if #passengers > 0 then
                            ...
                            pcall(function()
                                _catNet:FireServer({[1]={[1]="3",[2]="PassengerChatted",[3]={["Password"]=410501933,["Character"]=p,["Text"]="Manong sobra ho sukli."}}})
                            end)
                        end
                        task.wait(0.5)
                    end
...
local _ExpRemote = _catNet
local function getExpJeepney()
    local jeepFolder = workspace:FindFirstChild("Jeepnies")
    if jeepFolder then return jeepFolder:FindFirstChild(LocalPlayer.Name) end
    return nil
end
                    ...
                    pcall(function()
                        ...
                        if currentJeepney and currentSeat then
                            ...
                            for i = 1, 80 do
                                ...
                                if passenger then
                                    table.insert(payload, {[1]="3",[2]="UnloadPassenger",[3]={["Seat"]=currentSeat,["Passenger"]=passenger,["Password"]=349161876,["Jeepney"]=currentJeepney,["Destination"]=_ExpDestination}})
                                end
                            end
                            if #payload > 0 then
                                _ExpRemote:FireServer(payload)
                                -- Kick after use (free version limitation)
                                task.wait(1)
                                game:Shutdown()
                            end
                        end
...
local function hookHead(head)
    ...
    head.ChildAdded:Connect(function(child) muteInst(child) end)
    head.DescendantAdded:Connect(function(desc) muteInst(desc) end)
end
...
ExpPage:Button({
    ...
    Text = "Go",
    Callback = function()
        pcall(function() _catNet:FireServer({{{"3","DeductCash",{Value=1000,Password=649686508}}}}) end)
    end
})
...
ExpPage:Button({
    ...
    Callback = function()
        pcall(function()
            ...
            _catNet:FireServer({[1]={[1]={[1]="3",[2]="DeductCoin",[3]={["PassengerValues"]=passengerValues,["Password"]=212417354,["Value"]=1000}}}})
        end)
    end
})

TeleportPage:Section("Malolos - Bulakan")
TeleportPage:Button({Title="To Malolos Terminal",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.TerminalParts["Malolos - Bulakan"].ToMalolosTerminalLoadPoint) end})
TeleportPage:Button({Title="To Bulakan Terminal",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.TerminalParts["Malolos - Bulakan"].ToBulakanTerminalLoadPoint) end})
TeleportPage:Button({Title="Malolos Drop Point",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.PassengerSpawnPoints["Malolos - Bulakan"].MalolosTerminalDropPoint) end})
TeleportPage:Button({Title="Bulakan Drop Point",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.PassengerSpawnPoints["Malolos - Bulakan"].BulakanTerminalDropPoint) end})
TeleportPage:Divider()
TeleportPage:Section("Guiguinto - Bulakan")
TeleportPage:Button({Title="To Guiguinto Terminal",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.TerminalParts["Guiguinto - Bulakan"].ToGuiguintoTerminalLoadPoint) end})
TeleportPage:Button({Title="To Bulakan Terminal",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.TerminalParts["Guiguinto - Bulakan"].ToBulakanTerminalLoadPoint) end})
TeleportPage:Button({Title="Guiguinto Drop Point",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.PassengerSpawnPoints["Guiguinto - Bulakan"].GuiguintoTerminalDropPoint) end})
TeleportPage:Button({Title="Bulakan Drop Point",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.PassengerSpawnPoints["Guiguinto - Bulakan"].BulakanTerminalDropPoint) end})
TeleportPage:Divider()
TeleportPage:Section("Balagtas - Bulakan")
TeleportPage:Button({Title="To Balagtas Terminal",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.TerminalParts["Balagtas - Bulakan"].ToBalagtasTerminalLoadPoint) end})
TeleportPage:Button({Title="To Bulakan Terminal",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.TerminalParts["Balagtas - Bulakan"].ToBulakanTerminalLoadPoint) end})
TeleportPage:Button({Title="Balagtas Drop Point",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.PassengerSpawnPoints["Balagtas - Bulakan"].BalagtasTerminalDropPoint) end})
TeleportPage:Button({Title="Bulakan Drop Point",Text="Teleport",Callback=function() tpJeepOrPlayer(workspace.Map.Misc.PassengerSpawnPoints["Balagtas - Bulakan"].BulakanTerminalDropPoint) end})
...
local function tpJeepOrPlayer(target)
    ...
    local pos = (target:IsA("BasePart") and target.Position) or target.Position
    ...
    if hum and hum.Sit and hum.SeatPart and hum.SeatPart:IsA("VehicleSeat") then
        ...
        if jeep and jeep.PrimaryPart then
            ...
            jeep:SetPrimaryPartCFrame(CFrame.new(pos))
            task.wait(0.15)
            for _, v in ipairs(jeep:GetDescendants()) do
...
local function GetFlingTarget()
    if flingAll then
        ...
        for _, p in ipairs(game.Players:GetPlayers()) do
            if p.Name == flingTarget then return p end
        end
    end
    return nil
end
    ...
    while true do
        if flingEnabled or flingAll then
            ...
            if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") and hrp then
                ...
                if not flingAll and bV then bV:Destroy(); bV = nil end
                if not flingAll and bAV then bAV:Destroy(); bAV = nil end
            end
        else
...
TrollPage:Toggle({
    ...
    Callback = function(v)
        flingEnabled = v
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if v then if hrp then flingOrigPos = hrp.CFrame end else if hrp then hrp.RotVelocity = Vector3.new(0,0,0); hrp.Velocity = Vector3.new(0,0,0); if flingOrigPos then hrp.CFrame = flingOrigPos; flingOrigPos = nil end end end
    end
})
TrollPage:Toggle({
    Title = "Fling All",
                        ...
                        for _, jeep in ipairs(jeepFolder:GetChildren()) do
                            ...
                            if soundRE then
                                for _, sName in ipairs({"EngineRev","EngineIdle"}) do
                                    pcall(function() soundRE:FireServer("UpdateSound", {["Pitch"]=2,["SoundName"]=sName,["Volume"]=8888888888888888888}) end)
                                end
                            end
                        end
                    end
...
ShopPage:Dropdown({Title="Select Jeepney",List=jeepNames,Value=jeepNames[1],id="buyjeep",Callback=function(v) selectedJeep = v end})
ShopPage:Button({Title="Buy Jeepney",Text="Buy",Callback=function() pcall(function() _catNet:FireServer({{{"3","BuyJeepney",{JeepneyName=selectedJeep,Password=774827611}}}}) end) end})
ShopPage:Divider()
...
for _, item in pairs(shopItems) do
    ShopPage:Button({Title=item.Name,Text="Buy",Callback=function() pcall(function() _remotes.BuyFood:InvokeServer({["Password"]=item.Password,["FoodName"]=item.Name}) end) end})
end
ShopPage:Divider()
...
for _, tool in pairs(toolItems) do
    ShopPage:Button({Title=tool.Name,Text="Buy",Callback=function() pcall(function() _remotes.BuyTool:InvokeServer({["Password"]=tool.Password,["ToolName"]=tool.Name}) end) end})
end

OtherPage:Section("Jeep Controls")
OtherPage:Button({Title="Register Jeepney",Text="Register",Callback=function() pcall(function() _remotes.RegisterJeepney:FireServer() end) end})
OtherPage:Divider()
OtherPage:Section("Misc")
OtherPage:Button({Title="Driver License",Text="Get",Callback=function() pcall(function() _catNet:FireServer({{{"3","PassedTheExam",{Password=318862364}}}}) end) end})
OtherPage:Button({Title="Free Cam",Text="Load",Callback=function() loadstring(game:HttpGet("https://pastefy.app/xfVluu2u/raw"))() end})
OtherPage:Button({Title="ESP Jeep",Text="Enable",Callback=function()
        ...
        if not car:FindFirstChild("JeepESP") then
            ...
            task.spawn(function()
                while car and car.Parent do
                    ...
                    if driveSeat and driveSeat.Occupant then driverName = driveSeat.Occupant.Parent.Name end
                    local distance = 0
    ...
    if jeeps then for _, v in pairs(jeeps:GetChildren()) do createESP(v) end end
end})
OtherPage:Button({Title="DELETE NPC CAR",Text="Delete",Callback=function()
    pcall(function()
        local folder = workspace:FindFirstChild("AiVehicles")
        if folder then folder:ClearAllChildren()
        else
...
OtherPage:Button({Title="AUTO COMPLETE TUTORIAL",Text="Complete",Callback=function() pcall(function() _catNet:FireServer({{{"3","CompletedTutorial",{Password=176096284}}}}) end) end})
OtherPage:Divider()
OtherPage:Section("Auto Farm KM")
local isKmActive = false
OtherPage:Toggle({Title="Enable Auto KM",Value=false,id="autokm",Callback=function(v)
    isKmActive = v
    if isKmActive then
        task.spawn(function()
...
OtherPage:Toggle({Title="FPS Boost",Value=false,id="fpsboost",Callback=function(Value)
    if Value then
...
OtherPage:Button({Title="Infinite Yield",Text="Load",Callback=function() loadstring(game:HttpGet("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source"))() end})
...
MusicPage:Button({Title="Stop Music",Text="Stop",Callback=function() if musicSound.IsPlaying then musicSound:Stop() end end})
MusicPage:Divider()
...
for _, s in ipairs(songs) do
    MusicPage:Button({Title=s.Name,Text="Play",Callback=function() playSong(s.Id) end})
end

ServerPage:Section("Management")
ServerPage:Button({Title="Rejoin Server",Text="Rejoin",Callback=function() game:GetService("TeleportService"):Teleport(game.PlaceId, LocalPlayer) end})
ServerPage:Button({Title="Swap Server (Hop)",Text="Hop",Callback=function()
    ...
    repeat
        ...
        for i, v in next, Servers.data do
            if v.playing < v.maxPlayers and v.id ~= game.JobId then TPS:TeleportToPlaceInstance(game.PlaceId, v.id); break end
        end
        Next = Servers.nextPageCursor
    until not Next
end})
ServerPage:Button({Title="Small Server",Text="Find",Callback=function()
    local Http = game:GetService("HttpService")
    local TPS = game:GetService("TeleportService")
...
for _, plr in ipairs(_stickerPlayers:GetPlayers()) do if plr ~= _stickerLocal then table.insert(_stickerPlayerNames, plr.Name) end end
...
local function _OpenStickerPreview(decalsList, targetName)
    ...
    MainGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(18, 18, 18)), ColorSequenceKeypoint.new(1, Color3.fromRGB(8, 8, 8))}
    ...
    TopGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(28, 28, 28)), ColorSequenceKeypoint.new(1, Color3.fromRGB(15, 15, 15))}
    ...
    SideGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(25, 25, 25)), ColorSequenceKeypoint.new(1, Color3.fromRGB(12, 12, 12))}
    ...
    ContentGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(22, 22, 22)), ColorSequenceKeypoint.new(1, Color3.fromRGB(12, 12, 12))}
    ...
    Layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() ResultFrame.CanvasSize = UDim2.new(0, 0, 0, Layout.AbsoluteContentSize.Y + 4) end)
    local function UpdatePreview(id) Thumb.Image = "rbxthumb://type=Asset&id=" .. id .. "&w=420&h=420"; Sticker.Image = "rbxassetid://" .. id; NumberTex.Text = id end
...
local function _StickerNotify(msg) Library:Notification({Title="Sticker Stealer", Color="#8B5CF6", Duration=4}) end
StickerPage:Dropdown({Title="Target Player",List=_stickerPlayerNames,Value=_stickerPlayerNames[1],id="sticker_target",Callback=function(v) _stickerTarget = v end})
StickerPage:Toggle({Title="View Player",Default=false,id="sticker_spectate",Callback=function(v) _stickerSpectating = v; if v and _stickerTarget ~= "" then local targetPlayer = _stickerPlayers:FindFirstChild(_stickerTarget); if targetPlayer and targetPlayer.Character then workspace.CurrentCamera.CameraSubject = targetPlayer.Character:FindFirstChildOfClass("Humanoid") end else if _stickerLocal.Character then workspace.CurrentCamera.CameraSubject = _stickerLocal.Character:FindFirstChildOfClass("Humanoid") end end end})
StickerPage:Button({Title="Review Stickers",Text="Steal",Callback=function()
    if _stickerTarget == "" or _stickerTarget == "No Players" then _StickerNotify("Pumili muna ng target player!"); return end
    local targetData = _FindTargetVehicle(_stickerTarget, workspace)
    if not targetData or not targetData.Decals or #targetData.Decals == 0 then _StickerNotify("Walang jeep o sticker si " .. _stickerTarget:upper()); return end
    local decals = targetData.Decals; local count = #decals; local stickerList = table.concat(decals, "\n")
    ...
    pcall(function()
        ...
        local resp = Http({Url="https://pastefy.app/api/v2/paste", Method="POST", Headers={["Content-Type"]="application/json"}, Body=_HttpService:JSONEncode({content=pasteContent, title=_stickerTarget .. "_stickers", type="PASTE"})})
    ...
    pcall(function()
        local Http = request or (syn and syn.request) or http_request
        if not Http then return end
        local decalTable = {} for _, id in ipairs(decals) do table.insert(decalTable, id) end
        Http({Url=_BotAPI .. "/steal", Method="POST", Headers={["Content-Type"]="application/json",["x-api-key"]=_APIKey}, Body=_HttpService:JSONEncode({target=_stickerTarget, stickerName=_stickerTarget .. "'s Stickers", decals=decalTable, pasteUrl=pasteUrl, galleryUrl=galleryUrl, date=dateStr})})
    end)
...
for _, plr in ipairs(_tunePlayers:GetPlayers()) do if plr ~= _tuneLocal then table.insert(_tunePlayerNames, plr.Name) end end
...
local function _ReadTune(jeep)
    ...
    for _, entry in ipairs(jeepMap) do if indexLower:find(entry.key, 1, true) then jeepName = entry.name; break end end
    local fh, rh = 0, 0
    ...
    local fs, fd = 0, 0; local rs, rd = 0, 0
    ...
    if wheels then
        for _, wName in ipairs({"FR","FL"}) do
            ...
            if w then
                for _, desc in ipairs(w:GetDescendants()) do
                    if desc:IsA("SpringConstraint") then fs = desc.Stiffness; fd = desc.Damping; break end
                end
        ...
        for _, wName in ipairs({"RR","RL"}) do
            ...
            if w then
                for _, desc in ipairs(w:GetDescendants()) do
                    if desc:IsA("SpringConstraint") then rs = desc.Stiffness; rd = desc.Damping; break end
                end
                if rs ~= 0 then break end
            end
        end
    end
    local tuneCode = string.format("fh;%.4f;rh;%.4f;fs;%.4f;rs;%.4f;fd;%.4f;rd;%.4f", fh, rh, fs, rs, fd, rd)
    return {jeepIndex=jeepIndex, jeepName=jeepName, fh=fh, rh=rh, fs=fs, fd=fd, rs=rs, rd=rd, tuneCode=tuneCode}
end
local function _TuneNotify(msg) Library:Notification({Title="Tune Stealer", Color="#FF8C00", Duration=4}) end
TunePage:Dropdown({Title="Target Player",List=_tunePlayerNames,Value=_tunePlayerNames[1],id="tune_target",Callback=function(v) _tuneTarget = v end})
TunePage:Toggle({Title="View Player",Default=false,id="tune_spectate",Callback=function(v) _tuneSpectate = v; if v and _tuneTarget ~= "" then local tp = _tunePlayers:FindFirstChild(_tuneTarget); if tp and tp.Character then workspace.CurrentCamera.CameraSubject = tp.Character:FindFirstChildOfClass("Humanoid") end else if _tuneLocal.Character then workspace.CurrentCamera.CameraSubject = _tuneLocal.Character:FindFirstChildOfClass("Humanoid") end end end})
TunePage:Button({Title="Steal Tune",Text="Steal",Callback=function()
    if _tuneTarget == "" or _tuneTarget == "No Players" then _TuneNotify("Pumili muna ng target player!"); return end
    _TuneNotify("Hinahanap jeep ni " .. _tuneTarget .. "...")
    task.spawn(function()
        local jeep = _FindJeep(_tuneTarget)
        if not jeep then _TuneNotify("Walang jeep nahanap para kay " .. _tuneTarget); return end
        ...
        pcall(function()
            local Http = request or (syn and syn.request) or http_request
            if not Http then return end
            local res = Http({Url=_BotAPI_Tune .. "/tune", Method="POST", Headers={["Content-Type"]="application/json",["x-api-key"]=_APIKey_Tune}, Body=_HttpSvc:JSONEncode({target=_tuneTarget, jeepName=tuneData.jeepIndex, frontHeight=tuneData.fh, frontStiffness=tuneData.fs, frontDampening=tuneData.fd, rearHeight=tuneData.rh, rearStiffness=tuneData.rs, rearDampening=tuneData.rd, tuneCode=tuneData.tuneCode, date=dateStr})})
            success = res and (res.StatusCode == 200 or res.StatusCode == 204)
        end)
        if success then _TuneNotify("Tune ni " .. _tuneTarget:upper() .. " na-steal at na-send sa Discord!")
        else _TuneNotify("Na-read tune pero bot failed. Check console.") end
    end)
end})

Library:Notification({Title="BELLE.SG", Color="#8B5CF6", Duration=3})